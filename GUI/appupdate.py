...
def generate_questions(story_file):
    ...
    while j < count:
        ...
        num = 0
        marker = 0
        for i in range(i, min(i + 500, count)):  # Fix: Added min(i + 500, count) to avoid index out of range error
            red = 0
...
while True:
    ...
    if event == '-ABOUT-':
        about_layout = [
            ...
            [sg.Text('Gargi Shroff', font=('Arial', 12), justification='center'),
             sg.Text('Swarang Joshi', font=('Arial', 12), justification='center'),
             sg.Text('Ketaki Shetye', font=('Arial', 12), justification='center')],
            
        ]
        sg.popup_scrolled(layout=about_layout, title='About Us', size=(400, 200))
