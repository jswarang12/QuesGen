...
def generate_questions(story_file):
    ...
    # NOMINATIVE CASE
    questions = []
    output_list = [(word.text, word.upos, word.xpos, word.feats) for sent in doc.sentences for word in sent.words]  # Generate the output_list
    text = ""
    count = sum(1 for item in output_list if isinstance(item, tuple))
